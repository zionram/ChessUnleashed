...
const UNIVERSAL_PROMPT = `Create a valid Chess Unleashed experience package JSON based on my description.

Required top-level structure — these are the only allowed top-level keys:
{
...
- Do not add any top-level keys beyond format, metadata, contents, assetManifest (e.g. never add "theme", "assets", "config", "images" at the top level).
- Add content only inside "contents" (using the supported categories below) or inside "assetManifest.assets".
- Omit "assetManifest" only when the package has no package:// asset references; always include it when package:// refs are present.

Supported content categories — include ONLY the ones needed for my description:
...
- template: (optional) visual theme — include only the sub-fields you need:
    ...
    pieceThemeMode: "unified" (use pieceTheme) | "team" (use whitePieceTheme + blackPieceTheme)
    ...
    Use "" for all image fields unless referencing a file in the ZIP.
- audioSettings: { "masterVolume": 0–1, "musicVolume": 0–1, "sfxVolume": 0–1 }
- soundLibrary: [{ "id": "...", "name": "...", "url": "" }]
- soundRules: [{ "id": "...", "name": "...", "event": "...", "piece": "any", "side": "any", "mode": "any", "soundId": "..." }]
  Valid event: afterMove, pieceMoved, pieceCaptured, check, checkmate, promotion, gameStart, gameEnd, panelOpened
  soundId must reference a soundLibrary id
- customEvents: [{ "id": "...", "name": "...", "eventId": "...", "category": "custom", "baseTrigger": "...", "conditions": {}, "conditionMode": "all" }]
  Valid baseTrigger: same values as event above
- animationDefinitions: [{ "id": "...", "name": "...", "description": "...", "category": "...", "preset": "bounce", "targetType": "piece", "durationMs": 400, "delayMs": 0, "easing": "ease-in-out", "intensity": 0.7, "repeatCount": 1, "enabled": true, "status": "active" }]
...
- animationRules: [{ "id": "...", "eventId": "...", "animationId": "...", "target": "moved-piece", "scope": "any-piece", "enabled": true }]
  eventId must match a customEvents id; animationId must match an animationDefinitions id
  Valid target: moved-piece, captured-piece, target-square, source-square, board, current-player-panel
  Valid scope: any-piece, my-pieces, opponent-pieces, white-pieces, black-pieces
- timeControl, chatSettings, audioPlaylists, customRulesets, registeredBots, localProfile, multiplayerServer, personalityProfiles: include only if specifically needed

Hard rules:
- Before generating, use only fields that the current importer reads safely. Do not rely on this prompt's schema if it differs from the actual code. Do not include optional systems unless you are certain the importer supports them. Every array field you include must be initialized as an array (never null or undefined).
- Do NOT change field names or structure. The importer reads exact field names — renaming fields or restructuring objects causes silent import failures. If unsure, match the examples in this prompt exactly. Do not invent alternate shapes.
- NEVER include in contents: fen, moveHistory, currentTurn, timerState, result, gameState, boardState, historyFens
- NEVER invent field names not listed above
- For assets in the ZIP, use package://assets/<category>/<filename> (e.g. package://assets/pieces/pawn.png, package://assets/audio/effects/capture.mp3). Valid category values: pieces, boards, audio, ui. Use "" if no file will be provided.
- If any package:// references are used, include a top-level "assetManifest" object at the same level as "contents".
  assetManifest MUST be an object with an "assets" array — never a bare array:
    ✓ CORRECT:
      "assetManifest": {
        "assets": [
          { "id": "asset-1", "filename": "ocean-bg.jpg", "mimeType": "image/jpeg", "category": "boards",
            "packagePath": "assets/boards/ocean-bg.jpg", "originalDisplayName": "Ocean Background", "sourceUrl": "" }
        ]
      }
    ✗ WRONG — this shape will fail to import:
      "assetManifest": [ { ... } ]
  If unsure, copy the CORRECT structure above exactly. Do not invent alternate shapes.
  - "packagePath" is the in-package ZIP path (must start with assets/).
  - "sourceUrl" — include it whenever you know a real direct file URL; omit only when you do not:
    • A missing sourceUrl means the user must manually locate and provide that file. Include it whenever you can.
    • Actively seek public-domain or permissively-licensed files from these trusted domains:
        Images: upload.wikimedia.org, cdn.pixabay.com, raw.githubusercontent.com — prefer .png, .jpg, .jpeg, .webp
        Audio:  freesound.org, raw.githubusercontent.com — prefer .mp3 or .wav
    • Safety rules (a wrong URL breaks import — only include URLs you know are real):
      ✗ Must NOT be a webpage, search page, or GitHub file-browser page
      ✗ Must NOT be guessed, invented, or approximated — if uncertain, omit it
      ✗ URL must end in .png, .jpg, .jpeg, .webp, .mp3, or .wav and directly serve the file
    ✓ Entry with sourceUrl (app auto-fetches during ZIP export):
      { "id": "bg-1", "filename": "ocean-bg.jpg", "mimeType": "image/jpeg", "category": "boards",
        "packagePath": "assets/boards/ocean-bg.jpg", "originalDisplayName": "Ocean Background",
        "sourceUrl": "https://upload.wikimedia.org/wikipedia/commons/a/ab/Ocean.jpg" }
    ✓ Entry without sourceUrl (user must provide the file manually):
      { "id": "sfx-1", "filename": "capture-pop.mp3", "mimeType": "audio/mpeg", "category": "audio",
        "packagePath": "assets/audio/capture-pop.mp3", "originalDisplayName": "Capture Pop", "sourceUrl": "" }
- Use unique string ids for every array item
- Omit any category not relevant to the description

Output: the complete experience.json (the package JSON), then a "Required Assets:" section listing every image or audio file that still needs to be collected before packaging into a ZIP.

My description: [DESCRIBE WHAT YOU WANT HERE — replace this line]`;
