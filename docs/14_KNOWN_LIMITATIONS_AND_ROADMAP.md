...
- Obsidian built-in default theme: reset/default behavior should be smoke-tested after clearing saved settings and after editing/saving templates.
- Floating/docked windows: docking, undocking, close behavior, and launcher embedding should be regression-tested after any `App.tsx`, `FloatingWindow.tsx`, or `WorkspaceActionRegistry.tsx` changes.
- Move Assist: verify that the board-side assist window toggle controls the Black/White Assist overlay independently from move badges.
...
- The "Do Something Cool" Event Builder redesign direction is not fully complete unless verified in current code.
