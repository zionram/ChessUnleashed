...
const isPieceAnimationScopeAllowed = (
  ...
  if (scope === "my-pieces")
    return controlMode === "both" || movedColor === controlMode;
  if (scope === "opponent-pieces")
    return controlMode === "both" || movedColor !== controlMode;
  return true;
};
...
const ChessBoard: React.FC<ChessBoardProps> = ({
  ...
  if (!gameContext || !settings || !settings.template)
  ...
  const [movingPiece, setMovingPiece] = useState<MovingPieceAnimation | null>(
    null,
  );
  const [triggeredAnimation, setTriggeredAnimation] =
    useState<TriggeredAnimation | null>(null);
  ...
  const recentMoveAnimationRef = useRef<{
    key: string;
    timestamp: number;
  } | null>(null);

  const startMovingPieceAnimation = (
    sourceSquare: string,
    targetSquare: string,
    piece: BoardPiece,
  ) => {
  ...
  useEffect(
    () => () => {
      if (animationTimeoutRef.current)
        window.clearTimeout(animationTimeoutRef.current);
      if (triggeredAnimationTimeoutRef.current)
        window.clearTimeout(triggeredAnimationTimeoutRef.current);
    },
    [],
  );
  ...
  useEffect(() => {
    const handler = (event: any) => {
      ...
      triggeredAnimationTimeoutRef.current = window.setTimeout(
        () => {
          setTriggeredAnimation((current) =>
            current?.id === id ? null : current,
          );
        },
        Math.max(
          180,
          (payload.animation.durationMs || 300) +
            (payload.animation.delayMs || 0) +
            120,
        ),
      );
    };
  ...
  useEffect(() => {
    const handler = (event: any) => {
      ...
      if (settings.isThemeEditorMode || !settings.pieceAnimations.enabled)
        return;
      if (!payload.from || !payload.to) return;

      const movedColor: "w" | "b" =
        payload.team === "b" || payload.team === "black" ? "b" : "w";
      if (
        !isPieceAnimationScopeAllowed(
          settings.pieceAnimations.movementScope ?? "all",
          movedColor,
          controlMode,
        )
      )
      ...
      if (
        recentMoveAnimationRef.current &&
        recentMoveAnimationRef.current.key === moveKey &&
        Date.now() - recentMoveAnimationRef.current.timestamp <
          Math.max(180, settings.pieceAnimations.movementSpeedMs + 120)
      ) {
        return;
      }

      startMovingPieceAnimation(payload.from, payload.to, {
        type: pieceType,
        color: movedColor,
      });
    };
    ...
    settings.pieceAnimations.movementScope,
    settings.pieceAnimations.movementSpeedMs,
    controlMode,
  ]);
  ...
  useEffect(() => {
    ...
    const changedToSquares = Object.entries(currentBoardMap)
      .filter(
        ([square, piece]) => !piecesMatch(previousBoardMap[square], piece),
      )
    ...
    const source = target
      ? changedFromSquares.find(
          ({ piece }) =>
            piece.color === target.piece.color &&
            (piece.type === target.piece.type || piece.type === "p"),
        )
      : null;
    ...
    const wasCapture =
      !!previousBoardMap[target.square] &&
      previousBoardMap[target.square]?.color !== target.piece.color;
    const wasPromotion = source.piece.type !== target.piece.type;
    if (
      (wasCapture && !settings.pieceAnimations.captureAnimation) ||
      (wasPromotion && !settings.pieceAnimations.promotionAnimation)
    ) {
      setMovingPiece(null);
      return;
    }
    ...
    const scopeAllowsAnimation = isPieceAnimationScopeAllowed(
      scope,
      movedColor,
      controlMode,
    );
  ...
  const onSquareClick = (square: string) => {
    if (settings.isThemeEditorMode) {
      ...
      if (EDITOR_LAYOUT[square]) {
        window.dispatchEvent(
          new CustomEvent("chess-piece-selected", {
            detail: EDITOR_LAYOUT[square],
          }),
        );
      }
      return;
    ...
    if (ficsGame) {
      const ficsPlayerColor =
        ficsGame.relation === "playing-white"
          ? "w"
          : ficsGame.relation === "playing-black"
            ? "b"
            : null;
      const isFicsPlayerTurn =
        !!ficsPlayerColor && ficsGame.sideToMove === ficsPlayerColor;

      if (!ficsPlayerColor) {
        showIllegalMove("This FICS board is observe-only");
        setSelectedSquare(null);
        return;
      }

      const pieceAtSquare = currentBoardMap[square] ?? engine.getPiece(square);
      const isOwnPiece = pieceAtSquare?.color === ficsPlayerColor;

      if (!selectedSquare) {
        if (!isFicsPlayerTurn) {
          showIllegalMove("Waiting for your FICS turn");
          return;
        }
      ...
      const movingPiece =
        currentBoardMap[selectedSquare] ?? engine.getPiece(selectedSquare);
      if (movingPiece?.type === "p" && /[18]$/.test(square)) {
    ...
    if (selectedSquare === null) {
      ...
      if (piece && piece.color === engine.getGameState().turn) {
        ...
        if (controlMode !== "both" && piece.color !== controlMode) {
          showIllegalMove(
            `Mode restricted: Play as ${controlMode === "w" ? "White" : "Black"}`,
          );
          return;
        }
        setSelectedSquare(square);
      ...
      if (!move) {
        ...
        if (
          pieceAtTarget &&
          pieceAtTarget.color === engine.getGameState().turn
        ) {
  ...
  const squares = useMemo(() => {
    ...
    for (const r of rows) {
      for (const c of cols) {
        ...
        const piece = settings.isThemeEditorMode
          ? EDITOR_LAYOUT[squareName]
          : (currentBoardMap[squareName] ?? null);
        const pressure = settings.isThemeEditorMode
          ? { white: [], black: [] }
          : pressureMap[squareName] || { white: [], black: [] };
  ...
  const { boardOverlay } = visualTemplate;
  const overlayAnchor = ((boardOverlay as any).anchor || "center") as string;
  const overlayBlendMode = ((boardOverlay as any).blendMode ||
  ...
  const overlayBackgroundSize =
    overlaySize === "100% 100%" ? "100% 100%" : overlaySize;

  const getTriggeredAnimationName = (preset: string) =>
  ...
  const renderTriggeredAnimation = () => {
    ...
    const isBoardTarget =
      target === "board" ||
      animation.targetType === "board" ||
      animation.preset === "board-flash";
    const squarePosition = square
      ? getDisplayPosition(square, orientation)
      : { x: 43.75, y: 43.75 };
          ...
          <div className="cu-board-assist-score-row">
            ...
            <span
              style={{
                fontWeight: "bold",
                color:
                  displayScore(analysis.evaluation) >= 0
                    ? "#27ae60"
                    : "#e74c3c",
              }}
            >
              {displayScore(analysis.evaluation) > 0 ? "+" : ""}
              {displayScore(analysis.evaluation).toFixed(1)}
            </span>
          </div>
          <div>
            <div className="cu-board-assist-row">
              <span>Rec. Moves:</span>
              {gameContext.previewLine && (
                <button
                  onClick={() => gameContext.setPreviewLine(null)}
                  style={{
                    fontSize: "0.6rem",
                    padding: "0 3px",
                    cursor: "pointer",
            ...
            {analysis.topLines.map((item: any, i: number) => (
              <div
                ...
                style={{
                  ...
                  padding: "3px",
                  background:
                    gameContext.previewLine === item.line
                      ? "#e3f2fd"
                      : "transparent",
                  borderRadius: "3px",
                  cursor: "pointer",
              ...
              >
                ...
                >
                  ...
                  <span
                    style={{
                      color:
                        displayScore(item.score) >= 0 ? "#27ae60" : "#e74c3c",
                    }}
                  >
                    ({displayScore(item.score) > 0 ? "+" : ""}
