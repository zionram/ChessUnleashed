...
const AnimationPreviewCard: React.FC<AnimationPreviewCardProps> = ({ definition, compact = false }) => {
  ...
  if (!definition) {
    return (
      <div style={{ padding: 10, borderRadius: 8, border: '1px dashed #cbd5e1', color: '#64748b', fontSize: '0.75rem' }}>
  ...
  return (
    <div style={{ padding: compact ? 8 : 12, border: '1px solid #d0d7de', borderRadius: 8, background: '#f8fafc' }}>
      <style>{`
        ...
        @keyframes animation-preview-board-flash { 0%, 100% { background: #f8fafc; } 50% { background: #fde68a; } }
      `}</style>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', marginBottom: 8 }}>
        <div>
          <div style={{ fontWeight: 800, color: '#2c3e50', fontSize: compact ? '0.75rem' : '0.86rem' }}>{definition.name}</div>
          <div style={{ color: '#64748b', fontSize: '0.68rem' }}>{definition.preset} - {definition.durationMs}ms - {definition.enabled ? 'enabled' : 'disabled'}</div>
        </div>
        <button type="button" onClick={() => setPreviewKey(key => key + 1)} style={{ padding: compact ? '5px 7px' : '7px 10px', borderRadius: 6, border: '1px solid #d0d7de', background: '#fff', cursor: 'pointer', fontWeight: 700, fontSize: compact ? '0.68rem' : '0.75rem' }}>
          Test Preview
        </button>
      </div>
      <div style={{ height: compact ? 58 : 118, borderRadius: 8, border: '1px solid #e2e8f0', background: definition.preset === 'board-flash' ? '#f8fafc' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        <div key={previewKey} style={getPreviewStyle(definition)}>
          <div style={{ width: compact ? 28 : 42, height: compact ? 28 : 42, borderRadius: 7, background: definition.preset === 'board-flash' ? '#fde68a' : '#2c3e50', color: definition.preset === 'board-flash' ? '#2c3e50' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900 }}>
            A
          </div>
        </div>
      </div>
